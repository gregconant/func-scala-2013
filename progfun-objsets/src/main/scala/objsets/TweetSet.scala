...
class Empty extends TweetSet {
  ...
  override def mostRetweetedAcc(max: Tweet) = max
...
//  ((left union right) union that) incl elem
  ...
  override def descendingByRetweet: TweetList = {
    ...
    new Cons(biggest, this.remove(biggest).descendingByRetweet)
  }
  ...
  override def mostRetweetedAcc(max: Tweet): Tweet = {
    if(max.retweets < elem.retweets) {
      val leftMax = left.mostRetweetedAcc(elem)
      val rightMax = right.mostRetweetedAcc(elem)
      if(leftMax.retweets < rightMax.retweets) rightMax else leftMax 
    } else {
      val leftMax = left.mostRetweetedAcc(max)
      val rightMax = right.mostRetweetedAcc(max)
      if(leftMax.retweets < rightMax.retweets) rightMax else leftMax
    }
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.contains(tw.text))
